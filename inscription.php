...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($email)) {
        ...
        if ($result->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO users (email, password, verification_token, token_expiry, is_verified) VALUES (?, ?, ?, ?, 0)");
            $stmt->bind_param("ssss", $email, $hashed_password, $verification_token, $token_expiry);
            ...
            if ($stmt->execute()) {
                ...
                try {
                    ...
                    $verification_link = "http://localhost/blog1/complete_registration.php?token=" . $verification_token;
