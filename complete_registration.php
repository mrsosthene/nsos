...
if (strtotime($user['token_expiry']) < time()) {
    ...
    if (empty($prenom) || empty($nom) || empty($password) || empty($confirm_password)) {
        ...
        $update_stmt = $conn->prepare("UPDATE users SET prenom = ?, nom = ?, password = ?, is_verified = 1, verification_token = NULL WHERE id = ?");
        $update_stmt->bind_param("sssi", $prenom, $nom, $hashed_password, $user['id']);
