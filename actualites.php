...
$stmt = $pdo->prepare("SELECT * FROM articles WHERE statut = 'publie' ORDER BY date_publication DESC");
$stmt->execute();
...
<body class="bg-gray-100">
    ...
    <main class="container mx-auto px-4 py-8">
        ...
        <?php foreach($articles as $article): ?>
            <?php
            // Si l'article est premium et l'utilisateur n'est pas connecté, ajouter une classe de flou
            $blur_class = ($article['premium'] && !$is_logged_in) ? 'premium-blur' : '';
            ?>
            <article class="article-card mb-8 <?php echo $blur_class; ?>">
                <div class="relative">
                    <a href="<?php echo $is_logged_in || !$article['premium'] ? 'lire_article.php?id=' . $article['id'] : 'inscription.php'; ?>">
                        ...
                        <div class="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black to-transparent">
                            <h2 class="text-3xl font-bold text-white mb-2">
                                <?php echo stripslashes($article['titre']); ?>
                            </h2>
                ...
                <div class="p-6">
                    ...
                    <div class="flex justify-between items-center">
                        <div class="article-meta">
                            ...
                            <?php if($article['premium']): ?>
                            <span class="bg-yellow-500 text-white px-2 py-1 rounded text-sm ml-2">Premium</span>
                            <?php endif; ?>
                        </div>
                        <a href="<?php echo $is_logged_in || !$article['premium'] ? 'lire_article.php?id=' . $article['id'] : 'inscription.php'; ?>" 
                           class="read-more">
                            <?php echo $is_logged_in || !$article['premium'] ? 'Lire la suite →' : 'S\'inscrire pour lire →'; ?>
                        </a>
                    </div>
                </div>
            </article>
